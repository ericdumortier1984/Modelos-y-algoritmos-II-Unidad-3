...
public:
	...
	static b2MouseJoint* CreateMouseJoint(b2World* _world, b2Body* _second, float _maxForce)
	{
		b2MouseJointDef jointDef;

		jointDef.bodyA = CreateStaticBody(_world);
		jointDef.bodyB = _second;
		jointDef.target = _second->GetWorldCenter();
		jointDef.maxForce = _maxForce;

		b2MouseJoint* mouseJoint = (b2MouseJoint*)_world->CreateJoint(&jointDef);
		return mouseJoint;
	}
